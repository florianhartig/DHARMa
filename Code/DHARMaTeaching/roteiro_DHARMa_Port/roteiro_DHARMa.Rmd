...
Roteiro preparado para aula sobre diagnóstico de modelos lineares generalizados (mistos). 

Comentários e questões, por favor fale com [a autora](https://melinaleite.weebly.com/contact.html). 
...
```{r, message=F}
library(DHARMa)  # diagnóstico de modelos
library(lme4)    # ajuste de modelos mistos  
library(glmmTMB) # ajuste de modelos (mistos)
...
-   `Environment1` é a variável preditora ambiental, simulada de uma distribuição uniforme entre -1 e 1.\
-   `group` é a variável de agrupamento, que podem ser locais amostrados, regiões, blocos de experimentos, etc.\
...
Usamos a função `simulateResiduals`, que por padrão vai gerar 250 simulações^[para alguns casos vale apena aumentar o número de simulações], para calcular os **resíduos quantílicos**  para cada observação. Esses são os resíduos gerados pelo pacote DHARMa, para mais informações sobre como são calculados, veja a [vinheta do pacote](https://cran.r-project.org/web/packages/DHARMa/vignettes/DHARMa.html).
...
Interpretação do **gráfico da esquerda**:
...
Interpretação do **gráfico da direita**:

-   DHARMa **resíduos \~ preditos**. Esperamos uma distribuição completamente uniforme na direção _y_. Para isso, são feitas linhas de tendência com GAMs (generalized additive models) dos quantis ajustados nos resíduos em 0,25, 0,5, 0,75. Se for detectada alguma tendência, as linhas serão destacadas em vermelho (se você tiver dificuldade sem distinguir o vermelho, veja o help da função para mudar a cor de destaque).
...
Em `plotResiduals`, os valores preditos (eixo X), são geralmente transformados em ranking para facilitar observação de padrões, principalmente para distribuições cujas predições são distorcidas ("skewed"), mas podemos desligar se quisermos com o argumenton `rank=F`.


## Testes disponíveis

Os testes acima também pode ser realizados separadamente e com mais profundidade (+ gráficos adicionais) a partir das funções de cada teste.
...
Avaliando se os resíduos se conformam como uma distribuição uniforme (que é o resultado esperado para os resíduos quantílicos do `DHARMa`). Teste de Kolmogorov-Smirnov (KS).
```{r}
...
A função `testDispersion` no `DHARMa` possui diferentes teste de dispersão para diferentes tipos de modelos. Se quiser saber mais sobre testes de dispersão, leia [Leite et al. 2025](https://doi.org/10.32942/X23M14), e veja o help `?testDispersion` para mais detalhes.

O teste padrão do DHARMa, avalia se a variância dos valores observados da variável resposta é consistente com a variância esperada desta variável a partir de muitas simulações a partir do modelo. O histograma apresentado pelo teste é o da distribuição das variâncias dos valores simulados e a variância dos valores observados (linha horizontal vermelha). A estatística D é a razão entre o observado e esperado, com valores acima de 1 indicando sobredispersão e abaixo de 1 indicando subdispersão.
...
Testa se o número de _outliers_ (observações fora do envelope de simulação) é maior/menor do que seria esperado sob H0 (modelo corretamente especificado).
...
O gráfico mostra a distribuição dos resíduos e se houver outliers, eles serão destacados nos extremos 0 e 1 do eixo y.
...
Verificar se há mais zeros nos dados do que o esperado pela distribuição. O histograma mostra a quantidade de zeros vindo das simulações e a quantidade de zeros observada (linha vermelha vertical). O teste de zero-inflação é um teste de diferença entre a quantidade de zeros observada e a quantidade de zeros esperada pelo modelo.
...
Detecta se há indícios de autocorrelação espacial nos resíduos. Para isso, precisamos prover os valores das coordenadas dos dados. Aqui estamos usando as variáveis `x` e `y` do nosso conjunto de dados:
...
O mesmo para autocorrelação temporal, mas aqui precisamos de uma variável temporal (sequencia de acontecimentos). Vamos usar a variável `time` do nosso conjunto de dados:
...
Abaixo, apenas mostro um exemplo com outro conjunto de dados com autocorrelação filogenética gerados pelo pacote `phylolm`:

```{r, eval=F}
library(phylolm) # pacote de análise filogenética

  set.seed(123)
  tre = rcoal(60) # gerando uma árvore filogenética com 60 espécies
  # criando dados com autocorrelação filogenética:
  b0 = 0; b1 = 1;
  ...
  # ajustando modelo que ignora essa autocorrelação
  fit = lm(trait ~ pred, data = dat)
  
  # simulando resíduos do modelo
  res = simulateResiduals(fit, plot = T)

  # testando autocorrelação espacial dos resíduos
...
O modelo mais simples de todos é um glm Poisson (dados de contagem) com interação entre os preditores e um _offset_ para corrigir para o tamanho da ninhada.
