...
* Dispersion frequently varies with predictor variables (heteroscedasticity). This can have a significant effect on the inference. While it is standard to tests for heteroscedasticity in linear regressions, heteroscedasticity is currently hardly ever tested in GLMMs, although it is likely as frequent and influential. 
...
Power simulations (figure below) show that option 2 is generally not preferable. It is worse than even the uniform test. I only kept it in the package for testing and future improvements. 

Option 1 and 3 are substantially more powerful than the standard uniform test. The parametric alternative (option 1) has the advantage that it's fast, while option 3 might have slightly higher power and all other advantages of a non-parametric test, i.e. it should be still reliable for situations where distributional assumptions are violated
