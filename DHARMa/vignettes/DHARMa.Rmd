...
In principle this is no problem - each outcome is as likely as the other. However, there the somewhat unitutive behavior that, if you run a residual check several times, the result will always look different and will be sometimes more and sometimes less significant. See this example:
...
This can not only be confusing to a user, but it also bears the danger that one runs the plot several times and stops when it looks particularly nice. 

By default, DHARMa fixes therefore the random seed to the same value every time a simulation is run. This means that you will get exactly the same residual plot each time. If you want to avoid this behavior, for example for simulation experiments on DHARMa, use the setting "seed = NA" as above. 
