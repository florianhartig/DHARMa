...
author: "Florian Hartig, Theoretical Ecology, University of Regensburg [website](https://www.uni-regensburg.de/biologie-vorklinische-medizin/forschen/arbeitsgruppen/ag-hartig)"
date: "`r Sys.Date()`"
output:
...
\
**NOTE on plots**: The default color for highlighting outliers and significant tests is **red**. However, it can be changed by setting `options(DHARMaSignalColor = "red")` to a different color. This is convenient for a color-blind friendly display, since red and black are difficult for some people to disentangle.
...
The second option is the treatment of the stochastic hierarchy. In a hierarchical model, several layers of stochasticity are placed on top of each other. Specifically, in a GLMM, we have a lower level stochastic process (random effect), whose result enters into a higher level (e.g. Poisson distribution). For other hierarchical models, such as state-space models, similar considerations apply, but the hierarchy can be more complex. When simulating, we have to decide if we want to re-simulate all stochastic levels (**unconditional**), or only a subset of those. For example, in a GLMM, it is common to only simulate the last stochastic level (e.g. Poisson) conditional on the fitted random effects, meaning that the random effects are set on the fitted values (**conditional**).

**Advantages and disadvantages of unconditional / conditional simulations** 

|                                                    |                                                         |
|:---------------------------------------------------|:---------------------------------------------------|
| **Unconditional**                                  | **Conditional**                                       |
|✓ re-simulating all levels tests model structure as a whole |✗ residual checks do not check the entire model structure, only the final stochastic level; REs can absorb misfit, which will not be visible in conditional residuals |
|✓ implicitly test the normal distribution of REs |✗ do not test the normal distribution of REs | 
|✗ re-simulating lower-level REs creates more variability, which can reduce power for detecting problems in in upper-level stochastic processes (especially overdispersion, see below)         |✓ more sensitive for dispersion tests |

_For models with residual covariance structures (temporal, spatial, phylogenetic):_

|                                             |                                                         |
|:---------------------------------------------------|:---------------------------------------------------|
|✗ residuals will still show autocorrelation, even if the model has accounted for it correctly (solution: rotation, but this may take a long time and needs a high number of simulations for estimated rotation; see [here](https://github.com/florianhartig/DHARMa/issues/489)) |✓ allow to check directly whether residual autocorrelation structures have correctly been accounted for by the model; faster to compute  |
|✓ models with pure AR1 / OU processes do not produce patterns in unconditional residuals | ✗ for pure AR1 / OU processes, models may estimate very low residual variance, which can result in extreme patterns of the conditional residuals (see [here](https://github.com/florianhartig/DHARMa/issues/527)) | 
|                                             |                                                         | 

If the model is correctly specified and the fitting procedure is unbiased (disclaimer: GLMM estimators are not always unbiased), the simulated residuals should be flat regardless of how many hierarchical levels we re-simulate. The most thorough procedure would therefore be to test all possible options. In particular, we recommend dispersion tests to be used only for conditional simulations.
...
As of DHARMa 0.5.0, the **default is conditional simulation on all random effects of the fitted model**. Setting `simulateREs = "user-specified"` allows you to return to the previous DHARMa default, which used the respective default setting for the simulate function of your model class. Then, the `simulateResiduals` function allows users to pass on parameters to the simulate function of the fitted model object, using its respective syntax. It further allows specifying only a subset of random effects to be conditioned on, but note that this is not possible for all model classes (see table below). Conditional simulations are also currently not available for certain residual covariance structures (see section "Residual covariance structures").
...
| GLMMadaptive       | `type = "subject_specific"`, `new_RE = T` | `type = "subject_specific"` (default) |not available|
| mgcv               | not available           | default                      |not available|
| brms                | `re.form = NA`          | `re.form = NULL` (default)    | not available               |
...
Moreover (general advice), to ensure reproducibility, it's advisable to add a `set.seed()` at the beginning, and a `sessionInfo()` at the end of your script. The latter will list the version number of R and all loaded packages.
...
If you see this pattern, note that a common reason for underdispersion is overfitting, i.e. your model is too complex. Other possible explanations to check for include zero-inflation (best to check by comparing to a ZIP model, but see also `testZeroInflation`), non-independence of the data (e.g. temporal autocorrelation, check via `testTemporalAutocorrelation`) that your predictors can use to overfit, or that your data-generating process is simply not a Poisson process.
...
1.  *default: the simulation-based residual variance test* is a non-parametric test that compares the variance of the observed raw residuals to the variances of the simulated residuals.

2.  *PearsonChisq:* the Pearson-Chi2 test that is popular in the literature, suggested in the glmm Wiki, and implemented in some other R packages such as `performance::check_overdispersion`.

3.  *refit:* if residual simulations are done via refit and the test is **PearsonChisq**, DHARMa will compare the Pearson residuals of the re-fitted simulations to the original Pearson residuals. This is essentially a non-parametric version of test 2.
...
-   Option 2, the parametric Pearson-chi2, is fast if Pearson residuals are available, but based on a naive expectation of residual degrees of freedom (df, counts RE as 1 df) and the test statistic is thus biased towards underdispersion for mixed-effects models. Similar to the df approximation, bias increases with the number of RE levels. When testing only for overdispersion (`alternative = "greater"`), this makes the test more conservative, but it also costs power.
...
```{r, fig.height=5.5}
fittedModel <- glm(observedResponse ~ Environment1 + I(Environment1^2),
                   family = "poisson", data = testData)
...
Another way to visualize potential heteroscedasticity is to plot the residuals against the model predictors, using the `absoluteDeviation = TRUE`: it switches from displaying normal quantile residuals to absolute deviation from the mean expectation of 0.5 (calculated as `2 * abs(res - 0.5)`). 

```{r}
plotResiduals(simulationOutput, form= ~Environment1, absoluteDeviation = T)
```

To remove this pattern, you would need to apply a tranformation to the data or make the dispersion parameter dependent on a predictor, which can be done in `glmmTMB` with the `dispformula`:
```{r}
library(glmmTMB)
fittedModel2 <- glmmTMB(observedResponse ~ Environment1, 
                        dispformula = ~Environment1,
                        family = nbinom2(), 
                   data = testData)
simulationOutput2 <- simulateResiduals(fittedModel2)
plot(simulationOutput2)
plotResiduals(simulationOutput2, form= ~Environment1, absoluteDeviation = T)
```
...
OK, so let's add overdispersion through an beta-binomial distribution from `glmmTMB` package.
...
The next example uses the fairly well known Owl dataset which is provided in `glmmTMB` (see `?Owls` for more info about the data). The following shows a sequence of models, all checked with DHARMa. The example is discussed in a talk at ISEC 2018, see slides [here](https://www.slideshare.net/slideshow/mon-c5hartig2493/103596145).
