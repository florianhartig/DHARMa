...
```{r}
testData = createData(sampleSize = 500, intercept = 0, overdispersion = function(x){return(rnorm(length(x), sd = 4*abs(x)))}, family = poisson(), randomEffectVariance = 0)
...
## List of known residual patterns that are benign

As said in the introduction of this section: not all residual patterns are a sign of a misspecified model. Specifically, there are models that are known to deliver good estimates, even if the data don't conform to model assumptions, and there are models know to be slightly biased that nevertheless have such useful properties so that their use is indicated in many situations. Below a list of such examples, i.e. where one gets residual patterns that do not indicate a prboelm 

* MAR data https://github.com/florianhartig/DHARMa/issues/101
* MLE fits of REs https://github.com/florianhartig/DHARMa/issues/103
