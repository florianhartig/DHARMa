...
that basically do what they say. Outliers are again defined as observed data that are outside of what is produced by the simualations. The test checks if the number of outliers is higher than one would expect by chance. See the help of the functions and further comments below for a more detailed description.
...
1. The fact that a none of the here-presented tests shows as misspecification problem doesn't proof that the model is correctly specified. There are likely a large number of model misspecification problems that will not show a pattern in the standard residual plots

2. Moreover, while a clear pattern in the DHARMa residuals likely highlights that the observed data does not originate from 
...
```{r}
testData = createData(sampleSize = 500, overdispersion = 0.6, family = poisson())
fittedModel <- glmer(observedResponse ~ Environment1 + (1|group) , family = "poisson", data = testData)
