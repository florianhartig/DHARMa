...
The more interesting question is what happens in the low data situation. Starting from a "sharp" posterior that can be viewed as identical to the MLE, if we reduce the data, the first thing that will happen is that the simulated distribution will get wider for the Bayesian than for the MLE quantile residuals. Reducing the data even more, the posterior will shift to the prior. Thus, what we are doing approximately in these two idealized cases is either a) simulating data while sampling parameters from a normal distribution around the MLE b) simulation data while drawing parameters of the model from the prior. In either case, these situations will probably not create exactly uniform quantile residuals. 
