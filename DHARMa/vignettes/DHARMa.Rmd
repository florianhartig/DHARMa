...
abstract: "The 'DHARMa' package uses a simulation-based approach to create  readily
  ...
  'lme4' (classes 'lmerMod', 'glmerMod'), 'glmmTMB', 'GLMMadaptive', 'spaMM' and 'brms'; phylogenetic
...
```{r}
mod2 <- glmer(cbind(N_parasitized, N_adult) ~ logDensity + (1|ID), data = data,
              family=binomial)
...
```{r}
mod3 <- glmer(cbind(N_parasitized, N_adult) ~ logDensity + I(logDensity^2) +
