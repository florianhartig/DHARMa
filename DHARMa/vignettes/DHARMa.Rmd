...
As discussed above, for a correctly specified model we would expect asymptotically 
...
By default, plotResiduals plots against predicted values. However, if you can also use it to plot residuals against a specific other predictors (highly recommend) by using

```{r, eval = F}
plotResiduals(simulationOutput, form = YOURPREDICTOR)
```

If the predictor is a factor, or if there is just a small number of observations on the x axis, plotResiduals will plot a box plot instead of a scatter plot.

```{r, eval = F}
plotResiduals(simulationOutput, form = testData$group)
```

See ?plotResiduas for details, but very shortly: under H0 (perfect model), we would expect those boxes to range homogenously from 0.25-0.75. To see whether there are deviations from this expecation, the plot calculates a test for uniformity per box, and a test for homgeneity of variances between boxes. A positive test will be higlighted in red. 
...
Note, hover, that you will have to change the selection of variables that you provide to plots and tests (e.g. in plotResiduals or testSpatialAutocorrelation) accordingly when you group or subset residuals. 
...
1. A non-parametric test on the simulated residuals (default)
2. The parametric Pearson-chi2 test that is popular in the literature, suggested in the glmm Wiki, and implemented in some other R packages
3. A non-parametric overdispersion test based on the Pearson residuals of the re-fitted simulations, essentially a nonparametric version of test 2 

All of these tests are included in the testDispersion function, see help for details. 

```{r overDispersionTest, echo = T, fig.width=4.5, fig.height=4.5}
testDispersion(simulationOutput)
```

IMPORTANT: we have made extensive simulations, which have show that the various tests have certain advantages and disadvantages. The basic results are that: 
...
* The parametric Pearson-chi2 is fast, but based on a naive expectation of df and thus biased to underdispersion for mixed models, bias increasing with the number of RE levels. When testing only for overdispersion, this makes the bias conservative, but it also costs power. Overall, we do not see an advantage over option 1
* The DHARMa default option 1 is fast, nearly unbiased (i.e. you can test under and overdispersion), and only slightly less powerful as test 3, PROVIDED that simulations are made conditional on the fitted REs. Note that this is not the DHARMa default. 

Thus, the current recommendation is: for most users, use the default DHARMa test, but create simulations conditionally. See ?testDispersion for details. 
...
If a distance between residuals can be defined (temporal, spatial, phylogenetic), you should check if ther is a distance-dependence in the residuals, which would suggest to move to a gls structure for analysis. 

Two two functions to test for this in DHARMa are 
...
Once that is done, you should check for heteroscedasticity (via standard plot, also against all predictors), and for zero-inflation. As noted, zero-inflation tests are often negative, and rather show up as underdispersion. Work through the owl example below. 
...
There are a lot of rumors about what can and cannot be checked with binomial 0/1 data. Note that binomial data behaves slightly different when you have a 0/1 response than when you have a k/n response. 
...
DHARMa can also import external simulations from a fitted model via createDHARMa(), which will be interesting for unsupported packages and for Bayesians. Bayesians should note the extra Vignette on "DHARMa for Bayesians". 
