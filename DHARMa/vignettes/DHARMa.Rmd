...
1. **No residual pattern does not "proof" that the model is correct**: The fact that none of the DHARMa tests indicates a problem does not "proof" that the model is correctly specified. For each model, there are likely a large number of structural problems that will not create a pattern in the DHARMa diagnostics. In good old Popper fashion, you should only interpret no residual problems as your working hypothesis not having been rejected in this particular test, which increases confidence in the model, but does not constitute a decisive proof. So, keep your skepticism alive, and if you find the results fishy, keep searching and testing. 
...
```{r, error=TRUE}
m5 <- glmmTMB(SiblingNegotiation ~ FoodTreatment*SexParent + offset(log(BroodSize)) + (1|Nest), dispformula = ~ FoodTreatment + SexParent , ziformula = ~ FoodTreatment + SexParent,  data=Owls , family = nbinom1)
...
even though the misfit is clearly visible if we plot the residauls against the missing predictor.

```{r}
plotResiduals(simulationOutput, testData$Environment1, quantreg = T)
```

However, we can see the overdispersion arising from the misfit if we group our residuals, which basically basically transforms the 0/1 response in a k/n response. 

To show this, let's look at the dispersion test for the same model, once ungrouped (left), and grouped according the variable group which was in the the data (right).
