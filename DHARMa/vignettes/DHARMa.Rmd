...
DHARMa essentially performs a simulation experiment on the fitted model, to find out if the residuals fit to observation. A simulation means that random numbers are drawn, which meanst that results will be slightly different every time you do this.

This can not only be confusing to a user, but it also bears the danger that a user would run the simulation several times and take the result that looks better. Do not do this if you want to interpret your p-values properly - this is essentially multiple testing!

By default, DHARMa therefore fixes the random seed to the same value every time a simulation is run. This means that you will get exactly the same residual plot each time. If you want to avoid this behavior, for example for simulation experiments on DHARMa, use the setting "seed = NA" as above. 
...
Whether or not you fix the seed, the setting for the random seed and the random state are stored in 

```{r, eval = F}
simulationOutput$randomState
```

If you do not use a seed, the random state will still recorded. If you want to reproduce simualtions for such a run, set the variable .Random.seed by hand, and simulate with seed = NA. 
...
Note: previous versions of DHARMa (< 0.2.0) discouraged the simulated overdispersion test. I have since changed the code, and simulations show that it as powerful as the refitted or parametric test. Because of the generality and speed of this option, I see no good reason for refitting, and would recummend to run the standard test on the standard residuals. 
...
### General zero/k-inflation or deficits
...
```{r}
testData = createData(sampleSize = 500, overdispersion = 0, fixedEffects = 5, family = binomial(), randomEffectVariance = 0)
...
Moreover, if you would perform the DHARMa procedure per group, you should also be able to see overdispersion. Unfortunately, however, this option is not yet implemented in DHARMa.
...
lm and glm are fully supported as specified in the help

## lme4

lme 4 is fully supported as specified in the help

## glmmTMB

Current limitations of glmmTMB

* residual function doesn't work with factor response (glmmTMB bug)
* refit doesn't work with n/k data, because update doesnt' work   
* pearson residuals don't work with zi terms (not implemented), which limits some of the overdispersion tests
* Currently, glmmTMB doesn't support the reform argument. 
  ...
  * That means that all predictions and simulations are conditional on REs, which can sometimes create a positive correlation between res and predicted , see https://github.com/florianhartig/DHARMa/issues/43
