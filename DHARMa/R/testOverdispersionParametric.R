...
testOverdispersionParametric <- function(model){
  ...
  out = list (statistic=c(dispersion=prat, pearSS = Pearson.chisq, rdf=rdf), method = "Chisq test for overdispersion in GLMMs", alternative = "true dispersion greater 1", data.name = fittedModel@call$family)
  class(out) = "htest"
  return(out)
}
